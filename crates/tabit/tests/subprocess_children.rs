...
//! `subagent` tool with `execution: "subprocess"` spawns THIS
...
use tabit_session::{
    ChildRouter, EventFrame, ModelSelection, Session, SessionBuilder, SessionHost,
    SessionHostWiring, SessionStore, subagent,
};
...
/// subagent tool (`execution: subprocess`), and a second that wraps
...
"#,
    ...
    let parts = Arc::new(subagent::SubagentParts {
        ...
        tools: Vec::new(),
        max_turns: 8,
        model_factory: Arc::new(
            |_, _, _| -> Result<ModelHandle, tabit_session::SessionError> {
                unreachable!("the parent rides the scripted factory below, not this one")
            },
        ),
        router,
    ...
    let turns = vec![
        vec![
            MockStreamEvent::ToolCall {
                ...
                arguments: json!({"task": task, "execution": "subprocess"}),
                call_id: None,
            },
...
/// Read frames until a predicate holds; every frame is kept.
...
async fn a_subprocess_child_announces_streams_and_answers_over_the_real_binary() {
    ...
    let frames = drive_until(&mut handle, 60, |frame| {
        matches!(&frame.event, SessionEvent::RunFinished { output, .. } if output == "parent wrap-up")
    })
    .await;

    // The child announced through the same door, from the source: its
    // own stream stamp, the parent field, and the empty path of an
    // ephemeral session.
    let announcement = frames.iter().find_map(|frame| match &frame.event {
        SessionEvent::SessionOpened {
            id,
            path,
            parent: Some(parent),
            ..
        } if parent == &parent_id => Some((id.clone(), path.clone())),
        _ => None,
    });
    let announcement = match announcement {
        Some(found) => found,
        None => {
            let tool_results: Vec<String> = frames
                .iter()
                .filter_map(|frame| match &frame.event {
                    SessionEvent::ToolResult {
                        ...
                        status,
                        ..
                    } => Some(format!("{name} {status:?}: {content}")),
                    _ => None,
                })
                .collect();
            panic!("no child announcement; tool results: {tool_results:?}");
        }
    };
    let (child_id, child_path) = announcement;
