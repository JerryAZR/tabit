...
const PROVIDER_CASSETTE_SUITES: &[ProviderCassetteSuite] = &[
    ...
    // Vendored trim note: upstream also registers chatgpt, copilot, bedrock,
    // doubleword, gemini, ollama, llamafile, xai, openrouter, deepseek, groq,
    // mistral, perplexity, and mistralrs cassette suites. Those providers are
    // trimmed from this workspace; only anthropic + openai remain.
    ProviderCassetteSuite {
        provider: "anthropic",
