...
//!   and nothing else.** The author declares how each event kind is
//!   handled (the extension's watches, registered at serve or any
//!   time before a spawn), then spawns; there is no child-shaped
//!   registration surface (owner ruling 2026-09-25: registration is
//!   node/router-level config, and a lifetime hanging off the
//!   child's name was a naming-coincidence, not a mechanism). The
//!   child's own `session_opened` finds the watches already
//!   standing.
...
use tabit_protocol::{EventFrame, tags};
...
impl Child {
    ...
    /// recipe (prompt, then the settle fold), the same one the
    /// session's subagent tool rides. The child's asks surface per
    /// the policy while the run is in flight.
    pub async fn run(&mut self, task: String) -> Settlement {
        self.handle.prompt(task);
        self.handle.settle(None).await
    }
