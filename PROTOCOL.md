...
- **Routing is the framework, not the tool** (owner ruling). The
  ...
  exit.
- **Route all commands, no filters** (owner ruling: "`interaction_response` isn't a special command at the agent layer"). The router never decides — the target consumes. **Consumption is the session's own** (`SessionCommands` — one implementation over the session's handles, shared by the worker's dequeue point and the child router's in-process target), so every command consumes identically on either substrate. The correction round (owner challenge: "checkout/model/continue are already handled by sessions; if you need more code to make them work, something's wrong") deleted the first cut's in-process facade — a hand-rolled subset that rejected checkout/model/continue — and moved the beat: the parked checkout intent is session-resident, served by whichever driver holds the session (the worker loop between pumps; the pump between runs and at its exit — the pause point its own doc promised).
